...
# with open('./vocab_target.json', 'r') as json_file:
#     targetSeq_vocab = json.load(json_file)
targetSeq_vocabSize = len(targetSeq_vocab)
...
# 			      "t": 61, "y": 62}
with open('./vocab_drug.json', 'r') as json_file:
    drugSeq_vocab = json.load(json_file)
drugSeq_vocabSize = len(drugSeq_vocab)
...
def LabelDT(target_seqs, targetSeq_maxLen):
    label_targetSeqs = []
    targetSeq_truncated = targetSeq_maxLen
    # 变文本为数字
